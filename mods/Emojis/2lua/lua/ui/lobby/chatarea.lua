...
--requres mod installed
local emojies_textures = '/lua/ui/lobby/textures/'
local emojies = import('/lua/ui/lobby/emojies.lua').emojies or {}
...
ChatArea = Class(Group){
    ...
    PostMessage = function(self, messageText, authorName, messageStyle, authorStyle)
        ...
        local contents = self:WrapContents({
                                                text = self:CheckEmojies(messageText),
                                                style = messageStyle
                                            },
                                            {
                                                text = authorName,
        ...
        for _,content in contents do
            ...
            elseif content.emoji then--EMOJIES
                if parent then
                    ...
                    parent.child = Bitmap(parent,UIUtil.UIFile(emojies_textures .. content.emoji .. '.dds'))
                    LayoutHelpers.RightOf(parent.child, parent,2)
                    LayoutHelpers.AtVerticalCenterIn(parent.child, parent,lineSpacing)
                    ...
                    line.contents = Bitmap(line, UIUtil.UIFile(emojies_textures .. content.emoji .. '.dds'))
                    LayoutHelpers.RightOf(line.contents, line.name)
                    LayoutHelpers.AtVerticalCenterIn(line.contents, line.name,lineSpacing)
    ...
    CheckEmojies = function (self,text)
        local continue_flag
        local wasEmoji = true
        local result_table = {}
        for v in string.gfind(text, "[^:]+") do
            continue_flag = false
            for _, name in emojies do
